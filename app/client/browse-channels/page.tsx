...
export default function BrowseChannels() {
  ...
  async function createChannel() {
    ...
    const { data, error } = await supabase.rpc('create_channel_and_add_member', {
    ...
    if (error) {
      ...
      setChannels(prevChannels => [...prevChannels, { ...data, is_member: true }]);
      setNewChannelName('');
          ...
          <div key={channel.id} className="flex items-center justify-between py-2 border-b">
            ...
            {channel.is_member ? (
              <Button
                variant="outline"
                size="sm"
                onClick={() => router.push(`/client/channel/${channel.id}`)}
              >
                View
              </Button>
            ) : (
